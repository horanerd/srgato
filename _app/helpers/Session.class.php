 ...
 *Responsável pelas estatísticas, sessões e atualizações de tráfigo do sistema!
...
//           $this->sessionUpdate();
    ...
    //Obtém dados da tabela [HELPER TRAFIC]
    //ws_siteviews
    ...
    //Verifica, cria e atualiza o cookie do usuário [HELPER TRAFIC]
    private function getCookie(){
        $Cookie = filter_input(INPUT_COOKIE, 'useronline', FILTER_DEFAULT);
        setcookie("useronline", base64_encode("horanerd"), time() + 86400);
        if(!$Cookie):
            return false;
        else:
            return true;
    ...
    //Indetifica navegador do usuário!
    private function CheckBrowser() {
        ...
        elseif(strpos($this->Browser, 'MSIE') || strpos($this->Browser, 'Trindent/')  ):
            $this->Browser = 'IE';
        else:
            $this->Browser = 'Outros';
        endif;
    ...
    private function BrowserUpdate(){
        ...
        if(!$readAgent->getResult()):
            ...
            $upadateAgent = new Update;
            $upadateAgent->ExeUpdate('ws_siteviews_agent', $ArrAgent, "WHERE agent_name = :name", "name={$this->Browser}");
        endif;
    }
